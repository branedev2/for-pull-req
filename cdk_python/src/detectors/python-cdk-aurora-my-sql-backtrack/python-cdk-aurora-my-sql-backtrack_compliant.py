# Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
# SPDX-License-Identifier: MIT-0

# {fact rule=python-cdk-aurora-my-backtrack@v1.0 defects=0}
import aws_cdk as cdk
from aws_cdk.aws_rds import AuroraMysqlEngineVersion, DatabaseCluster, DatabaseClusterEngine

class CdkStarter(cdk.Stack):
    def __init__(self, scope: cdk.App, id: str):
        super(scope, id)

        # Compliant: Aurora MySQL cluster has backtrack enabled with a 30-second window, allowing point-in-time recovery of data.
        DatabaseCluster(self, 'rDbCluster', 
          engine=DatabaseClusterEngine.aurora_mysql(version=AuroraMysqlEngineVersion.VER_3_01_0),
          backtrack_window=30
        )
# {/fact}
