...
# {fact rule=terraform-public-read-bucket-acl@v1.0 defects=0}
resource "aws_s3_bucket" "private_bucket" {
  bucket = "example-private-bucket"
  # Compliant: The AWS S3 bucket ACL does not grant READ permission to any users.
  acl = "private"
  lifecycle_rule {
     id     = "expire"
     status = "Enabled"
...
resource "aws_s3_bucket_replication_configuration" "east_to_west" {
    ...
    rule {
      status = "Enabled"
      destination {
        bucket        = aws_s3_bucket.private_bucket.arn
