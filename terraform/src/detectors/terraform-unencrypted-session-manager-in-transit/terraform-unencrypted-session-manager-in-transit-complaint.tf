...
# {fact rule=terraform-unencrypted-session-manager-in-transit@v1.0 defects=0}
resource "aws_ssm_document" "session_manager_compliant" {
  ...
  content = jsonencode({
    ...
    inputs = {
      ...
      # Compliant: Session Manager data is encrypted during transit.
      kmsKeyId                   = aws_kms_key.session_key.arn
    }
  })
}
# {/fact}